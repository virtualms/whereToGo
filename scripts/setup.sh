...
cd $SCRIPTS
echo -n "Insert new dataset name: "
read
dt_name=${REPLY}
./dt_tree.sh $dt_name &
...
ls $dt_name | tr " " "\n" | while read phase; do
	...
	ls ./$dt_name/$phase | tr " " "\n" | while read section; do
		if test $section == "images"; then
			ls "$SOURCE/images" | tail -n $num_imgs | while read img; do
				cp $SOURCE/images/$img ./$dt_name/$phase/$section
			done
		else
			ls "./$dt_name/$phase/images" | sed "s/.jpg/.txt/g" | while read label; do
				cp $SOURCE/labels/$label ./$dt_name/$phase/$section
			done
		fi
	done
done
...
echo "Starting data augemntation:"
cd $SCRIPTS

# Filters application
if ! test -z $filters 2>/dev/null ; then
